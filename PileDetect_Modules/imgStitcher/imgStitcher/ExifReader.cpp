...
std::vector<std::string> ExifReader::splitString(std::string str, std::string pattern)
{
...
void ExifReader::readImageLonLatFromTxt(string &filename, vector<double> &longitude, vector<double> &latitude)
{
	char tmp[100];
	int num = 0;
	...
	for (int i = 0; i < num; i++)
	{
		...
		infile >> tmp;
		location = splitString(string(tmp), pattern);
		longitude.push_back(atof(location[0].c_str()));
		latitude.push_back(atof(location[0].c_str()));
